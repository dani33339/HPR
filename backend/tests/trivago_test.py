...
class TestTrivagoScraper(unittest.TestCase):
    ...
    def test_add_deal_to_hotel_dict(self):
        # Mock data
        deal = {
            'advertiser': {
                'nsid': {
            ...
            'clickoutUrl': 'http://example.com/deal'
        }
        partner_id_dict = {
            '123': 'partner_123'
        }
        ...
        expected_hotel_dict = {
            'partner_123': {
                'channel': 'trivago',
                'partner': 'partner_123',
                'price': round(1500 / currency_helper.get_exchange_rate("INR"))*trip_length, 
                'url': 'http://example.com/deal'
            }
        }

        trivago.add_deal_to_hotel_dict(deal, partner_id_dict, hotel_dict, trip_length)
        self.assertEqual(hotel_dict, expected_hotel_dict)


if __name__ == "__main__":
    unittest.main(defaultTest='TestTrivagoScraper.test_get_prices_and_links')
